      ...
      <div className="max-w-[1500px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="relative w-full bg-[#0d0d0d] rounded-[2.5rem] lg:rounded-[3rem] px-6 py-12 md:p-12 lg:p-16 flex flex-col lg:flex-row gap-12 lg:gap-16 border border-white/[0.03] shadow-2xl overflow-hidden">
          ...
          <div className="lg:w-[40%] xl:w-[45%] flex flex-col justify-start relative z-10">
            ...
            <p className="text-text-muted text-base lg:text-[17px] leading-relaxed mb-10 lg:mb-12 max-w-sm">{t('subtitle')}</p>
            ...
            <div className="flex flex-col gap-6">
              ...
              <a href="mailto:support@belkbodylab.com" className="flex items-center gap-5 group">
                <div className="w-12 h-12 rounded-full bg-[#161616] border border-white/5 flex items-center justify-center text-text-muted transition-all shadow-md group-hover:border-accent-green group-hover:text-accent-green">
                  <svg width="20" height="20" fill="currentColor" viewBox="0 0 256 256"><path d="M224,48H32A16,16,0,0,0,16,64V192a16,16,0,0,0,16,16H224a16,16,0,0,0,16-16V64A16,16,0,0,0,224,48ZM203.43,64,128,133.15,52.57,64ZM224,192H32V74.83l90.59,83.1A16,16,0,0,0,133.41,158L224,74.83Z"></path></svg>
                </div>
          ...
          <div className="lg:w-[60%] xl:w-[55%] flex flex-col gap-4 z-20">
            {faqs.map((faq, i) => (
              <div key={i} className="bg-[#161616] rounded-2xl border border-white/[0.04] transition-all overflow-hidden shadow-lg">
                <button onClick={() => setOpenIndex(openIndex === i ? null : i)} className="w-full flex items-start justify-between px-6 py-6 lg:p-7 text-left gap-4 cursor-pointer focus:outline-none">
                  <div className="flex items-start gap-4 lg:gap-6">
                    <span className="font-display text-lg lg:text-xl lg:pt-0.5 text-[#5e616b] font-medium leading-[1.3] lg:leading-[1.4]">{String(i + 1).padStart(2, '0')}</span>
                    <span className={`font-semibold text-sm md:text-base lg:text-[17px] leading-relaxed transition-colors duration-300 ${openIndex === i ? 'text-white' : 'text-white/80'}`}>{faq.q}</span>
                  </div>
                ...
                <div className={`faq-answer ${openIndex === i ? 'open' : ''}`}>
                  <div className="px-6 py-4 pt-0 lg:px-7 lg:pb-7 flex gap-4 lg:gap-6">
                    <span className="text-transparent font-display text-lg lg:text-xl select-none leading-[1.3] lg:leading-[1.4]" aria-hidden="true">{String(i + 1).padStart(2, '0')}</span>
                    <p className="text-[#a0a0a0] text-[13px] lg:text-[14px] leading-relaxed pr-6 md:pr-10">{faq.a}</p>
                  </div>
                </div>
              </div>
            ))}
