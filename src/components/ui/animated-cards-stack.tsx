  ...
  (
    {
      ...
      incrementY = 10,
      incrementZ = 10,
      incrementRotation = index % 2 === 0 ? 2 : -2,
      className,
      variant,
      style,
      ...props
    ...
    const range = React.useMemo(() => [start, end], [start, arrayLength, index])
    const rotateRange = [range[0], range[1]]

    const y = useTransform(scrollYProgress, range, ["0%", "-120%"])
    ...
    const alpha = useTransform(scrollYProgress, rotateRange, [0.15, 0])
    const filter =
      variant === "light" 
