...
if (!fmMatch) fmMatch = mdContent.match(/^---\s*([\s\S]*?)\s*---/);
...
const postObj = {
  ...
  es: {
    ...
    metaTitle: esTranslation.title,
    metaDescription: esTranslation.excerpt,
    content: esTranslation.htmlContent,
    faqs: faqsEs,
    customSchemas: schemas
  }
};
...
if (arrayEndIndex !== -1) {
  ...
  console.log("Successfully appended fully translated blog post ID 25 to posts.ts");
} else {
