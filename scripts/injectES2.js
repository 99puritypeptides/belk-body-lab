...
// Now replace the `es: { ... }` block for ID 12 and 14 Safely!
function replaceESBlock(id, newEsObj) {
  ...
  // Find the exact closing brace of the `es: {` block by counting braces
  let braceCount = 0;
  let esEnd = -1;
  let inString = false;
  let escape = false;
  for (let i = esStart; i < contentStr.length; i++) {
      ...
      if (char === '"' || char === "'" || char === "\`") {
          // Simplistic string handling, assuming no mixed quotes issues in the current block
          // Let's do something simpler: we know `customSchemas:` is right after it.
      }
