...
// Since the schemas array contains JSON strings, JSON.stringify escaped their quotes.
// In TypeScript, we can keep them as string literals but unescape the backslashes 
// OR we can just let JSON.stringify handle it and it's valid string. 
// However, JSON.stringify also escapes newlines as \n. This is perfectly valid TS.
...
if (arrayEndIndex !== -1) {
  ...
  console.log("Successfully appended blog post to posts.ts");
} else {
