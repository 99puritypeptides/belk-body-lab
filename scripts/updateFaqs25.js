...
const content = fs.readFileSync('C:/Users/kaise/.gemini/antigravity-ide/scratch/belk-body-lab/src/data/blog/posts.ts', 'utf8');

// We will find the specific faqs arrays for post 25
// Since the file is huge and regex might be risky, let's locate `slug: "peptide-reconstitution-calculator-guide"`
const enIdx = content.indexOf('slug: "peptide-reconstitution-calculator-guide"');
if (enIdx !== -1) {
  // Find the faqs array for EN
  const enFaqsIdx = content.indexOf('faqs: [', enIdx);
  const endEnFaqsIdx = content.indexOf(']', enFaqsIdx);
  
  const beforeEn = content.substring(0, enFaqsIdx);
  const afterEn = content.substring(endEnFaqsIdx + 1);
  
  let newContent = beforeEn + 'faqs: ' + JSON.stringify(faqsEN, null, 4) + afterEn;
  
  // Now find the ES slug
  const esIdx = newContent.indexOf('slug: "peptide-reconstitution-calculator-guide"', endEnFaqsIdx);
  if (esIdx !== -1) {
    const esFaqsIdx = newContent.indexOf('faqs: [', esIdx);
    const endEsFaqsIdx = newContent.indexOf(']', esFaqsIdx);
    
    const beforeEs = newContent.substring(0, esFaqsIdx);
    const afterEs = newContent.substring(endEsFaqsIdx + 1);
    
    newContent = beforeEs + 'faqs: ' + JSON.stringify(faqsES, null, 4) + afterEs;
    
    fs.writeFileSync('C:/Users/kaise/.gemini/antigravity-ide/scratch/belk-body-lab/src/data/blog/posts.ts', newContent);
    console.log("Successfully updated FAQs for post 25.");
  } else {
    console.log("ES slug not found.");
  }
} else {
  console.log("EN slug not found.");
}
