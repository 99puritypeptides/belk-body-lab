...
// Custom JSON stringifier to safely unquote keys
function formatObject(obj) {
  let str = JSON.stringify(obj, null, 2);
  // Unquote top-level and nested keys if they match typical identifier format
...
// Now replace the `es: { ... }` block for ID 12 and 14
function replaceESBlock(id, newEsObj) {
  const start = contentStr.indexOf(`id: "${id}"`);
  if (start === -1) {
  ...
  // The es block ends when the next key starts or at `}` before `},`
  // To be safe, we can look for `customSchemas:` which is right after `es: { ... }`
  const schemaStart = contentStr.indexOf('customSchemas:', esStart);
  
  // We want to replace from `es: {` up to `customSchemas:`
  // Or we find the exact closing `},` of the es block.
  // We can just format the new ES block and slice it in.
  
  let formattedES = 'es: ' + formatObject(newEsObj);
  // Indent everything inside formattedES
  formattedES = formattedES.split('\n').map((l, i) => i === 0 ? l : '  ' + l).join('\n') + ',\n  ';
  
  // Replace from esStart to schemaStart
  contentStr = contentStr.substring(0, esStart) + formattedES + contentStr.substring(schemaStart);
  console.log(`Replaced ES block for post ${id}`);
}

replaceESBlock('12', es12);
replaceESBlock('14', es14);
